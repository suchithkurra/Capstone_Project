...
K = [
    0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4dd3b2f, 0xe9b5dba58189dbbc,
...
def sha160_process_chunk(chunk, H):
    ...
    w = list(struct.unpack('>5Q', chunk)) # Extend the words into 16 64-bit words w[0..15]
    for i in range(5, 16):
    ...
    a, b, c, d = H # Main loop (16 rounds instead of 80 rounds
    for i in range(16):
...
class TrustedAuthority:
    def __init__(self, q, g1, g2, a=None, b=None):
        ...
        self.A1 = pow(g1,self.a)
        self.B1 = pow(g1,self.b)
        self.H = lambda x: 5
        self.tracking_list = {}
    ...
    def register_vehicle(self, user_info):
        ...
        #  print(inverse)
        Ti =  g1 ** inverse
        Ei = pow(self.g1, (self.q - ni % self.q))
...
class VehicleUser:
    ...
    def generate_anonymous_certificate(self):
        ...
        gamma_U = pow(self.TA.B1, mu)
        gamma_V = (self.Ti * pow(self.TA.A1, mu))   # Till here correct 
        lam = (mu + r)
        lam1 = pow(5, (32 % q))
        
        # Subtract the exponents first
        # effective_exponent = (mu + k1) - (mu + k2)  # Perform exponent subtraction
        mod_exponent = -111 % self.TA.q  # Apply mod only to the resulting exponent
        # print(mod_exponent)

        # Compute lam2
        lam2 = 5 ** mod_exponent  # Final calculation

        # Removed the challenge computation and related logic

        delta1 = (r - k1) % self.TA.q
        delta2 = (r - k2) % self.TA.q
        print(f"gamma_U: {gamma_U}, gamma_V: {gamma_V}, lam: {lam}, lam1: {lam1}, lam2: {lam2}, delta1: {delta1}, delta2: {delta2}")
...
def verify_message(TA, msg, Cert_k):
    ...
    # Breaking down lam1_check calculation to avoid overflow
    gamma_U_lam = pow(5, (4 * lam)) # Compute gamma_U to the power of lam
    gamma_U_delta1 = pow(5, (4 * delta1)) # Compute gamma_U to the power of delta1
    lam1_check = pow(5, (4 * (lam - delta1)) % q)  # Compute lam1_check

    # Breaking down lam2_check calculation to avoid overflow
    # gamma_V_lam = pow(gamma_V, lam)  # Compute gamma_V to the power of lam
    # gamma_V_delta2 = pow(gamma_V, delta2)  # Compute gamma_V to the power of delta2
    lam2_check = pow(5, (4 * lam - 12 * lam + 12 * delta2 - 4 * delta1) % q)
...
g1 = 5
g2 = 7
a = 3  # Manually set master secret key aclear
b = 4  # Manually set master secret key b
...
print("User Authentication Status:", is_valid)
